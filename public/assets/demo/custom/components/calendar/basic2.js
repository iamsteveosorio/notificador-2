    ...
    init: function () {
      ...
      $("#m_calendar2").fullCalendar({
        ...
        header: {
          ...
          center: "title",
          right: "mytimelineWeek",
          // "timelineDay,mytimelineWeek,timelineMonth,timelineYear,listWeek",
        },
        editable: true,
        ...
        loading: function (bool) {
          ...
          else mApp.unblockPage(); // Add your script to show loading
        },
        resourceRender: function (resourceObj, $td) {
          ...
          // return ['all', resourceObj.id].indexOf($('#empleadoSelector').val()) >= 0
        },
        dayRender: function (date, cell) {
          if (
            diasFestivos.some(
              (dia) => dia.fecha === moment(date).format("YYYY-MM-DD")
            )
          ) {
            ...
            $(".fc-head")
              .find("[data-date=" + moment(date).format("YYYY-MM-DD") + "]")
              .addClass("alert-danger");
            $(".fc-head")
              .find("[data-date=" + moment(date).format("YYYY-MM-DD") + "]")
              .find(".fc-cell-text")
              .html(dia.descripcion);
          }
        },
        ...
        eventRender: function (event, element) {
          if (element.hasClass("fc-timeline-event")) {
            ...
            var resource = $("#m_calendar2").fullCalendar(
              "getResourceById",
              event.resourceId
            );
            element
            ...
            element
              .find(".fc-list-item-title")
              .append(
                '<div class="m--font-boldest m--font-danger">' +
                  resource.title +
                  "</div>"
              );
          }
          // return ['all', event.resourceId].indexOf($('#empleadoSelector').val()) >= 0
        },
            ...
            success: function (response) {
              ...
              $("body #m_repeater_1").repeater({
                ...
                show: function () {
                  $(this).slideDown();
                  $(
                    "body #m_timepicker_1, body #m_timepicker_1_modal"
                  ).timepicker({
                    defaultTime: "08:00:00",
                    minuteStep: 15,
                    showMeridian: false,
